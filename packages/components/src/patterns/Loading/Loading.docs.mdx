...
The `IressLoadingSuspense` component works with React 19's built-in `use` hook to handle loading states. Create your promises outside of the render function so they remain stable references, then pass them to `use()`.

```tsx
import { use } from 'react';

// Create the promise once so it's a stable reference across renders.
const homePagePromise = API.fetchPage('home');

const HomePage = () => {
  const pageData = use(homePagePromise);
