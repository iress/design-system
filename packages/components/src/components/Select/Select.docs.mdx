...
For multi-select, pass an array of values to `defaultValue` and each will be resolved to its matching option.

<ComponentExample of={ComponentStories.PreSelectedMultiValue} />

If you need a controlled component (where the selection cannot be changed by the user), pass a `LabelValueMeta` object to the `value` prop instead.
