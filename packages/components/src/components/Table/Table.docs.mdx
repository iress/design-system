...
Filtering can be enabled on a column by setting the `filter` property of the `TableColumn` to `true`. A filter icon will appear in the column header, and clicking it opens a popover with the unique column values as checkboxes. Users can select one or more values to filter the table rows.

To pre-filter a column on initial render, use the `defaultFilter` property with an array of string values. The user can still clear or change the filter interactively.

The filter button has visually hidden text for screen readers to signify that the column is filterable. If you need to change this (e.g. for translation purposes), use the `filterableText` column option which defaults to: filterable.

By default, the filter dropdown displays the raw cell values. To customise the display of filter options, use `formatFilter` on the column. When not provided, it falls back to the column's `format`. This is useful when the column uses rich formatting (e.g. pills or icons) and you want the filter dropdown to match.
