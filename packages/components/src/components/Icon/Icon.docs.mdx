...
From version 6, the `IressIcon` component uses Material Symbols icons from Google as pre-generated SVG components.

#### Basic Usage

Simply use the `name` prop with any Material Symbol icon name:
...
The icons will automatically load on first use and be cached for future use. There is no need to manually import or register icons.
