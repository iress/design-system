...
<ComponentExample of={ComponentStories.InputModes} meta={ComponentStories} />
...
<ComponentExample of={ComponentStories.Clearable} meta={ComponentStories} />
...
<ComponentExample of={ComponentStories.TextAreas} meta={ComponentStories} />
...
The `disabled` prop can be set to disable the input. This will change the input to a custom disabled style and the user will not be able to interact with the input.

It is recommended to use the `disabled` prop on the `IressField` component as wellas on the `IressInput` component to ensure all associated elements (like label) are also disabled.

**Note:** For accessibility reasons, this prop does not set a `disabled` attribute on the native input element, to allow screen readers to still read the value.

<ComponentExample of={ComponentStories.Disabled} meta={ComponentStories} />
...
<ComponentExample of={ComponentStories.Formatter} meta={ComponentStories} />
