...
import * as ProviderStories from './TooltipProvider.stories';
import {
  ComponentExample,
  ComponentOverview,
  TestIdTable,
...
### Using the `IressTooltipProvider`

You can use the `IressTooltipProvider` to set a shared container for
all nested tooltips. This is useful when you want all tooltips in a
section of your application to render into the same DOM node.

Individual tooltips can still override the provider's container by
setting their own `container` prop.

> **Note:** If you are already using `IressProvider` or `IressShadow`,
> you do not need to add `IressTooltipProvider` separately — it is
> already included. To set a shared container for tooltips through
> `IressProvider`, use the `tooltipContainer` prop.

<ComponentExample
  api={{
    details:
      'These are some settings you can use to customise the provider, such as setting where the tooltips are rendered in the DOM.',
    heading: 'Props',
...
<TestIdTable testIds={testIds} testIdPrefix="my-tooltip" />
