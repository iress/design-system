...
- Conveying brief messages about background processes or status changes. Instead use [IressAlert](?path=/docs/components-alert--docs) or [IressToaster](?path=/docs/components-toaster--docs).
...
<ComponentExample of={ComponentStories.ShowWithState} />
...
<ComponentExample of={ComponentStories.FixedFooter} />
...
<ComponentExample of={ComponentStories.Size} />
...
<ComponentExample of={ComponentStories.ResponsiveSize} />
...
For instances where you require the user's full attention and you want to
ensure they continue the current task before closing the modal, you can use
`disableBackdropClick={true}` to disable backdrop clicks while still allowing
Escape key dismissal. If you need to disable both backdrop clicks and Escape
dismissal, set `disableBackdropClick="no-esc"`. You can also hide the close
button using `noCloseButton`.
...
<ComponentExample of={ComponentStories.DisableClosing} />
...
<ComponentExample of={ComponentStories.V5ModalDiff} />
...
<TestIdTable testIds={testIds} testIdPrefix="my-modal" />
