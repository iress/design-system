...
IDS intentionally does not support the `disabled` pattern on buttons. Disabled buttons are invisible to screen readers and provide no way for users to understand how to enable the action.
