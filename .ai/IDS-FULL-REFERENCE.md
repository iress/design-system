...
> **Pattern:** `import { IressLoading } from '@iress-oss/ids-components'`
> **Storybook:** [Loading in Storybook](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/docs/components_patterns-loading--docs)
...
<IressLoading screenReaderText="Loading..." />
```
...
<IressRow gutter="md">
  <IressCol span={6}>
    ```tsx
<LoadingSuspenseWizard />
```

[View "Suspense" example in Storybook →](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/story/components_patterns-suspense--suspense)
  </IressCol>
  <IressCol span={6}>
    ```tsx
<LoadingSuspenseWizardFast />
```

[View "FastSuspense" example in Storybook →](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/story/components_patterns-suspense--fast-suspense)
  </IressCol>
</IressRow>
```
...
The `IressLoadingSuspense` component works with React 19's built-in `use` hook to handle loading states. Use `useRef` to create a stable promise per component instance — this ensures the request starts when the component mounts and each instance has its own independent promise, rather than sharing a single cached promise across all instances.

```tsx
import { use, useRef } from 'react';

const HomePage = () => {
  // Create the promise once per component instance.
  // Using useRef ensures the request starts on mount and is not shared across instances.
  const homePagePromiseRef = useRef(API.fetchPage('home'));
  const pageData = use(homePagePromiseRef.current);
...
> **Legacy polyfill**
>
> The `IressLoadingSuspense.use` and `IressLoadingSuspense.uncache` static
>   methods are still available as a polyfill for React 18 applications. If you
>   are on React 19, use the built-in `use` hook instead.
...
[View "Wizard" example in Storybook →](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/story/components_patterns---wizard)
  </IressCol>
  <IressCol span={6}>
    ```tsx
<LoadingWizardFast />
...
[View "FastWizard" example in Storybook →](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/story/components_patterns---fast-wizard)
  </IressCol>
</IressRow>
```
...
*View interactive examples: [https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/docs/components_patterns-loading--docs](https://main--691abcc79dfa560a36d0a74f.chromatic.com/?path=/docs/components_patterns-loading--docs)*
