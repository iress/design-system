...
> **⚠️ DO NOT SKIP THIS STEP.** Storybook is the consumer-facing documentation site. Due to Storybook's flakiness, every component MUST be visually verified in the browser. Code-level checks alone are insufficient — the rendered output can differ from what the code suggests. Each check below must be performed for every component in the batch.

For each component, navigate to its docs page and verify ALL of the following:

1. [ ] **Playground tab renders**: Component is visible (not blank/errored), interactive controls table is present below the story
2. [ ] **Playground code panel**: Toggle "Show code" and verify:
   - No `container: {}` (should show `container: document.body` or be absent)
   - No `<p />` or other empty/broken elements
   - No `_react` or internal React references
   - Shows clean JSX with `@iress-oss/ids-components` imports (not `@/main`)
...
6. [ ] **API (Props) tab**: Exists and shows the props table
7. [ ] **Breakpoint stories** (if applicable): Show the breakpoint label decorator above the story (e.g. "xs", "sm", "md")
8. [ ] **No console errors**: Check browser console for React errors, missing imports, or rendering failures related to the component
