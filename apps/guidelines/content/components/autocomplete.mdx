...
<IressAutocomplete limitDesktop={4} limitMobile={3} options={...} />

// 4 results on all screens (limitMobile > limitDesktop so has no effect)
<IressAutocomplete limitDesktop={4} limitMobile={5} options={...} />
```

The result limits will also affect the number of initial options displayed, if the `initialOptions` prop is set.

```tsx
import { IressAutocomplete, IressPanel, IressStack } from '@iress-oss/ids-components';
