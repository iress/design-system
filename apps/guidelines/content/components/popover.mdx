...
Popovers can contain a wide variety of different content, including components that use popovers themselves (like Select). This can cause issues with overflowing content, so by default we allow popovers to grow in height based on their content.
