...
<IressInput placeholder={`${type} input`}
type={type}
key={type}
/>
))}
...
<IressInput placeholder={`${inputMode} mode`}
inputMode={inputMode}
key={inputMode}
/>
))}
...
<IressInput append={<IressIcon name="search" />}
placeholder="Append slot"
/>

<IressInput prepend={<IressIcon name="mail" />}
placeholder="Prepend slot"
/>

<IressInput append={<IressIcon name="calendar_today" />}
placeholder="Append slot"
/>
</IressStack>
```
...
<IressInput
  ...
  actions={[{ children: 'Copy', icon: 'content_copy' }]}
/>
```
...
```tsx
<IressInput
  placeholder="Enter a string and it will show in UPPERCASE when not focused"
...
<IressInput
  ...
  type="number"
  formatter={(value) => new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD' }).format(Number(value))}
/>
```
