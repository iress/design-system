...
<IressInput placeholder="Enter a string and it will show in UPPERCASE when not focused, and show the raw value on focus" formatter={(value) => String(value).toUpperCase()} />
```
...
<IressInput defaultValue={0.00} placeholder="Enter any number and it will show in currency format when the input is not focused" type="number" formatter={(value = '') => {
